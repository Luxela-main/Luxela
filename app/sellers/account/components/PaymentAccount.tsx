...
export function PaymentAccount({ initialData }: PaymentAccountProps) {
  ...
  const updatePaymentMutation = (trpc.seller as any).updateSellerPayment.useMutation({
    onSuccess: () => {
      toastSvc.success("Payment account updated successfully");
      (utils.seller as any).getProfile.invalidate();
    },
    onError: (error: any) => {
      toastSvc.error(error.message || "Failed to update payment account");
    },
  });
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-lg p-6">
      <h2 className="text-xl font-medium mb-6">Payment Account</h2>
      ...
      <form onSubmit={formik.handleSubmit} className="space-y-6">
        <div>
          ...
          <select
            ...
            className="w-full bg-[#222] border border-[#333] rounded-md px-3 py-2 text-white focus:border-purple-600 focus:ring-purple-600 cursor-pointer"
          >
