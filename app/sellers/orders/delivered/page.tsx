...
export default function DeliveredOrdersPage() {
  ...
  const deliveredOrders = useMemo(
    () => allOrders.filter((order: Sale) => order.orderStatus === "delivered"),
    [allOrders]
  )

  const filteredOrders = useMemo(() => {
    let result = deliveredOrders.filter((order: Sale) => {
      return (
        filters.search === "" ||
  ...
  return (
    <div className="px-4 md:px-6 mt-4 md:mt-0">
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Delivered Orders</h1>
          <p className="text-gray-400 mt-2 text-sm">
            {isLoading ? "Loading..." : `${filteredOrders.length} order${filteredOrders.length !== 1 ? "s" : ""} successfully delivered`}
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title={filters.search ? "No orders match your search" : "No delivered orders"}
          description={filters.search ? "Try adjusting your search criteria" : "Delivered orders will appear here"}
        />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filteredOrders.map((order: Sale) => (
            <Link key={order.id} href={`/sellers/orders/${order.orderId}`}>
              <div className="bg-[#1a1a1a] border border-[#333] rounded-lg p-5 hover:border-green-500/50 transition cursor-pointer">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <Badge className="bg-green-900/30 text-green-200 border-green-700/30 border">
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div>
                    <p className="text-xs text-gray-500 uppercase">Delivered Date</p>
                ...
                <Button size="sm" className="w-full bg-green-600 hover:bg-green-700 text-white cursor-pointer">
                  View Order →
                </Button>
              </div>
            </Link>
