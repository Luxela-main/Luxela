...
import type { Order, ProductCategory, PaymentMethod, PayoutStatus, OrderStatus, DeliveryStatus } from '@/types/buyer';
import { Search, AlertCircle, Loader, Truck, ArrowLeft } from 'lucide-react';
...
export default function ShippedOrdersPage() {
  ...
  // Fetch shipped orders from backend
  ...
  useEffect(() => {
    if (ordersData?.data) {
      const mappedOrders: Order[] = ordersData.data.map((item: any) => ({
        ...
        paymentMethod: (item.paymentMethod || 'credit_card') as PaymentMethod,
        amountCents: item.priceCents || item.amountCents || 0,
        currency: item.currency || 'NGN',
        payoutStatus: (item.payoutStatus || 'in_escrow') as PayoutStatus,
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-white text-2xl font-semibold pb-0">Shipped Orders</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-[#1a1a1a] rounded-lg p-12 text-center">
          <p className="text-gray-400 text-lg">No shipped orders found</p>
        </div>
      ) : (
        <>
          <div className="space-y-4">
            {paginatedOrders.map((order) => (
              <Link key={order.orderId} href={`/buyer/dashboard/orders/shipped/${order.orderId}`}>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-[#212121]">
                    <div className="flex items-center gap-8 flex-wrap">
                      ...
                      {order.trackingNumber && (
                        <div>
                          <span className="text-[#7e7e7e] text-sm">Tracking:</span>
                          <span className="text-white font-medium ml-2">{order.trackingNumber.slice(0, 12)}</span>
                        </div>
                      )}
                    </div>
                  </div>
