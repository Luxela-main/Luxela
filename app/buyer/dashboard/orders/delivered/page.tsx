...
import { Search, AlertCircle, Loader, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-white text-2xl font-semibold pb-0">Delivered Orders</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-[#1a1a1a] rounded-lg p-12 text-center">
          <p className="text-gray-400 text-lg">No delivered orders found</p>
        </div>
      ) : (
        <>
          <div className="space-y-4">
            {paginatedOrders.map((order) => (
              <Link key={order.orderId} href={`/buyer/dashboard/orders/delivered/${order.orderId}`}>
                <div className="bg-[#1a1a1a] rounded-lg p-6 border border-[#212121] hover:border-[#8451e1] transition cursor-pointer">
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-white text-sm">
                      Delivered{' '}
                      {order.deliveredDate
                        ? order.deliveredDate.toLocaleDateString('en-US', {
                            month: 'long',
                            day: 'numeric',
                            year: 'numeric',
                          })
