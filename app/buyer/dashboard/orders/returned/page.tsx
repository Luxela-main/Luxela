...
import { Search, AlertCircle, Loader, XCircle, ArrowLeft } from 'lucide-react';
...
export default function ReturnedOrdersPage() {
  ...
  const { data: ordersData, isLoading: isDataLoading, error: queryError, refetch } = trpc.buyer.getPurchaseHistory.useQuery(
    { status: 'canceled', page: currentPage, limit: itemsPerPage },
  ...
  useEffect(() => {
    ...
    if (queryError) {
      setError('Failed to load returned/canceled orders.');
    }
    ...
    <div>
      <Breadcrumb
        items={[
          ...
          { label: 'Returned & Canceled Orders' },
        ]}
      />
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-white text-2xl font-semibold pb-0">Returned & Canceled Orders</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-[#1a1a1a] rounded-lg p-12 text-center">
          <p className="text-gray-400 text-lg">No returned or canceled orders found</p>
        </div>
      ) : (
        <>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <span className="text-white text-sm">
                        {order.deliveredDate
                          ? `Delivered ${order.deliveredDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`
                          : `Ordered ${order.orderDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`}
                      </span>
