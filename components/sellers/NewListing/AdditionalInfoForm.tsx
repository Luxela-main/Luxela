...
<ImagePreview images={formData.images} />
      ...
      <div className="col-span-12 lg:col-span-7 text-sm space-y-6">
            ...
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-lg border transition ${
                formData.targetAudience === 'unisex'
                  ? 'bg-purple-700border-[#333] text-white'
                  : 'bg-transparent border-[#333] text-gray-500'
              }`}
            >
              Unisex
        ...
        <div>
          ...
          <div className="flex gap-3">
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  formData.shippingOption === 'local' ? 'border-purple-600' : 'border-[#333]'
                }`}>
                  {formData.shippingOption === 'local' && (
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  formData.shippingOption === 'international' ? 'border-purple-600' : 'border-[#333]'
                }`}>
                  {formData.shippingOption === 'international' && (
                    <div className="w-2.5 h-2.5 rounded-full bg-purple-600"></div>
                  )}
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  formData.shippingOption === 'both' ? 'border-purple-600' : 'border-[#333]'
                }`}>
                  {formData.shippingOption === 'both' && (
                    <div className="w-2.5 h-2.5 rounded-full bg-purple-600"></div>
                  )}
        ...
        <div>
          <label className="block text-sm font-medium mb-4">Estimated shipping time</label>
          
          {/* Within Country */}
          <div className="mb-4">
            <label className="block text-sm text-gray-400 mb-2">Within country</label>
            <div className="flex gap-3">
              <input
                type="text"
                placeholder="00 Days"
              ...
              <input
                ...
                value={formData.domesticMinutes}
          ...
          {/* International */}
          <div>
            <label className="block text-sm text-gray-400 mb-2">International</label>
            <div className="flex gap-3">
              <input
                type="text"
                placeholder="00 Days"
