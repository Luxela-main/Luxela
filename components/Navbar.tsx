  ...
  return (
    <nav className={`z-50 transition-all duration-300 text-white ${sticky ? 'sticky top-0 left-0 right-0 bg-[#0A0A0A]/80 backdrop-blur-md' : 'relative bg-transparent'}`}>
      <div className="container z-10 mx-auto px-2 md:px-10 py-4 flex justify-between items-center">
        ...
        <ul className="hidden md:flex items-center space-x-9">
          {navItems.map((item) => (
            <li key={item.name + item.route}>
              <Link href={item.route} className="text-xs lg:text-sm hover:text-purple-500 transition">
                {item.name}
              </Link>
            </li>
          ))}
        ...
        {/* Hamburger */}
        <button className="cursor-pointer md:hidden text-white" onClick={() => setMobileMenuOpen(true)}>
...
const MobileNav = ({ setMobileMenuOpen }: { setMobileMenuOpen: (open: boolean) => void }) => {
  return (
    <div className="md:hidden flex flex-col fixed top-0 w-screen h-screen bg-[#0E0E0E] z-40 px-6 py-4">
      ...
      <div className="flex items-center mt-5 w-full">
        <Link href="/" className="w-[132px] h-[21px] mx-auto ">
          <Image src={"/images/Luxela-white-logo-200x32.svg"} width={132} height={32} className="w-full h-full" alt="Luxela logo" />
        </Link>
        <button className="size-9 text-white ml-auto cursor-pointer" onClick={() => setMobileMenuOpen(false)}>
          <X className="size-6" />
        </button>
      </div>
      ...
      <ul className="mt-10 flex flex-col space-y-6 text-sm">
        {navItems.map((item) => (
          <li key={item.name + item.route}>
            <Link href={item.route} onClick={() => setMobileMenuOpen(false)} className="flex items-center hover:text-purple-500 transition text-[1.5rem]">
              <span>{item.name}</span> <ChevronRight className='ml-auto hover:text-[#8451E1] mr-10'/>
            </Link>
          </li>
        ))}
      </ul>

      <Link href="#" className="mt-auto h-[42px] flex items-center justify-center space-x-2 bg-purple-500 transition text-white rounded-[10px] px-6">
        <span className=''>Shop now</span><ShoppingCart className="ml-2 h-4 w-4" />
      </Link>
    </div>
  );
};
