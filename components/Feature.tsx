    ...
    <section id="feature" className="z-10 py-20 px-4">
      <main className="grid gap-40 container max-w-6xl mx-auto">
        {features.map((feature, index) => (
          <div
            key={feature.title}
            className="max-w-[1240px] mx-auto min-h-[720px] p-6 bg-[#1A1A1A] rounded-[20px] flex flex-col lg:flex-row items-center gap-[30px]">
            <div data-aos={`${index / 2 === 0 ? "flip-down" : "flip-up"}`}>
              <h3 className="text-lg lg:text-[2rem] font-semibold text-[#F9F9F9]">
                {feature.title}
              </h3>
              <p className="text-sm lg:text-lg text-[#BFBFBF] mt-9">
                {feature.description}
              </p>
            </div>
            <Image
